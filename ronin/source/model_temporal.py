...
class LSTMSeqNetwork(torch.nn.Module):
    ...
    def forward(self, input, hidden=None):
        if hidden is None:
            hidden = self.init_weights(input.size(0))  # Initialize hidden state based on input batch size
        else:
            hidden = (hidden[0].unsqueeze(1), hidden[1].unsqueeze(1))  # Adjust hidden state for current batch size
